...
#docker run --rm --name appsvr -p 80:8080 -v $(pwd)/build/libs/template-artifactid.war:/usr/local/tomee/webapps/template-artifactid.war tomee:8-jre-7.0.4-plume

# Java EE 8 servers
#docker run --rm --name appsvr -p 80:8080 -v $(pwd)/build/libs/template-artifactid.war:/opt/jboss/wildfly/standalone/deployments/template-artifactid.war jboss/wildfly:13.0.0.Final /opt/jboss/wildfly/bin/standalone.sh --server-config=standalone-full.xml -b "0.0.0.0" -Dee8.preview.mode=true
